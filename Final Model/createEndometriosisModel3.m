function m1 = createEndometriosisModel3(nva)
arguments
    ...
    nva.ssNO  = 46%*1e3;          % nM - not sure
    

    % Turnover rates
    nva.trARG   = 2;       % hours
    nva.trARGi   = 1;       % hours
    nva.trNO  = 5.5e-7;          % hour
...
%Channel parameter affinity 
nva.kinChON = nva.kinChOFF/2;
...
ed.addspecies('IGF10',       'Value', 15.6883,               'Units', 'nanomolarity');
...
ed.addspecies('EC',       'Value', 0,               'Units', 'nanomolarity');
...
ed.addspecies('NO0',            'Value', nva.ssNO,             'Units', 'nanomolarity');
...
m1.addparameter('EmaxNO',    'Value', 1,              'Units','dimensionless');
m1.addparameter('EC50NO',   'Value', 1,              'Units','nanomolarity');
% Drug
...
ed.addspecies('EP30',     'Value', nva.ssEP3,             'Units', 'nanomolarity');
...
% PGE2 with IGF1 uptake
m1.addparameter('EmaxPGE2',    'Value', 1,              'Units','dimensionless');
m1.addparameter('EC50PGE2',   'Value', 1,              'Units','nanomolarity');
% add reactions
% PGE2 synthesis and degradtion 
m1.addreaction('null -> PGE2',   'ReactionRate', 'ksynPGE2*((1+ (EmaxPGE2 * (IGF1/EC50PGE2 )))/(1+  (IGF1/EC50PGE2 )))',        'Name', 'PGE2 synthesis');
...
m1.addparameter('rpIGF1',   'Value', nva.rpIGF1,              'Units','nanomolarity/hour');
...
m1.addparameter('rpARG',   'Value', nva.rpARG,              'Units','nanomolarity/hour');
...
%-------------------------------------------------------------
%flux  Jflux = 
% where no of channels = f(IGF1) ; 
% k =k(literature)* f(NO + EP3); GmaxFlux(literature) * No of channels* (conc/(conc+k(affinity of pump for ca2+));
%Flux
m1.addparameter('JFlux', 'Value', 0, 'Units','nanomolarity/hour', ...
   'Constant', 0);
m1.addparameter('JFlux0', 'Value', 8.88e+6, 'Units','nanomolarity/hour', ...
   'Constant', 0);
m1.addparameter('GmaxFlux', 'Value', 14.8*1000 , 'Units','nanomolarity/hour', ...
   'Constant', 0); %14.81 microMoles/s = 14.81*1000/(1/3600)=53316000
m1.addparameter('Ca', 'Value', 1000, 'Units','nanomolarity', ...
   'Constant', 0);
%-------------------------------------------------------------
%Affinity of pump for ca2+ ions
m1.addparameter('Kaff0', 'Value', 0, 'Units','nanomolarity', ...
   'Constant', 0);  %0.6*1000
% m1.addparameter('KaffDelay', 'Value', 0, 'Units','nanomolarity', ...
%    'Constant', 0);
ed.addspecies('Kaff',      'Value', 0,              'Units', 'nanomolarity');
m1.addparameter('hcPain',20,'Units','dimensionless', ...
   'Constant', 0);
m1.addparameter('kaffBasal',1,'Units','nanomolarity/hour');
m1.addparameter('EmaxPainCh',    'Value', 1,              'Units','dimensionless');
m1.addparameter('EC50PainCh',   'Value', 10,              'Units','nanomolarity');
% m1.addparameter('kinChON', 'Value', nva.kinChON, 'Units','nanomolarity/hour', 'Constant', 0);%INCLUDE delay due to on and off of pump by EP3 and NO
m1.addparameter('koutChOFF', 'Value', 1, 'Units','1/hour', 'Constant', 0);%0.8 was arbitrarily picked to give a delay that would take hours to take that
 m1.addparameter('alpha',   'Value', 0.5,              'Units','dimensionless');
addrule(m1, 'Kaff0 = (alpha * EP3 + (1-alpha)*NO)', 'RuleType', 'repeatedAssignment');
 %-------------------------------------------------------------
 m1.addreaction('null  -> Kaff  ',...
     'ReactionRate', 'kaffBasal*EmaxPainCh * (Kaff0/EC50PainCh) ^hcPain/(1 + (Kaff0/EC50PainCh)^hcPain)',...
      'Name', 'Activation of Pump by EP3 and NO');
 m1.addreaction('Kaff -> null', 'ReactionRate', 'koutChOFF*Kaff',...
      'Name', 'Deactivation of Pump by EP3 and NO');
%-------------------------------------------------------------
...
%-------------------------------------------------------------
%No of channels
ed.addspecies('Nchannels',      'Value', 1,              'Units', 'nanomolarity');
ed.addspecies('NchSig',      'Value', 1,              'Units', 'nanomolarity');
m1.addparameter('NchBasal',1,'Units','nanomolarity/hour');
m1.addparameter('EmaxPainIGF1',    'Value', 1,              'Units','dimensionless');
m1.addparameter('EC50PainIGF1',   'Value', 10,              'Units','nanomolarity');
m1.addparameter('gamma',   'Value', 2,              'Units','dimensionless');
m1.addparameter('kinChIGF1', 'Value', 0.8, 'Units','nanomolarity/hour');%INCLUDE delay due to gene expression to kin0 = Ebase*kout ebase of Cchannels is 100nM
m1.addparameter('koutChIGF1', 'Value', 0.8, 'Units','1/hour');%https://pubmed.ncbi.nlm.nih.gov/16759698/ from this paper its 0.8....but have reduced to 0.08
m1.addparameter('koutChSig', 'Value', 1, 'Units','1/hour');
m1.addparameter('ScaleCh', 'Value', 1, 'Units','nanomolarity'); %reflect the kout in order of days
%---------------------------------------------------------------------------------
m1.addreaction('null  -> Nchannels  ',...
    'ReactionRate', 'kinChIGF1 * (IGF1/(IGF10))^gamma',...
     'Name', 'Ca2+ ion channel synthesis');
m1.addreaction('Nchannels -> null', 'ReactionRate', 'koutChIGF1*Nchannels',...
     'Name', 'Ca2+ ion channel clearance');
%-------------------------------------------------------------
% m1.addreaction('null  -> NchSig  ',...
%     'ReactionRate', 'NchBasal * EmaxPainIGF1 * Nchannels/(Nchannels + EC50PainIGF1 )',...
%      'Name', 'Ca2+ ion channel activation');
% m1.addreaction('NchSig -> null', 'ReactionRate', 'koutChSig*Nchannels',...
%      'Name', 'Ca2+ ion channel deactivation');
%-------------------------------------------------------------
addrule(m1, 'NchSig = ScaleCh* 1 * Nchannels/(Nchannels + EC50PainIGF1 )', 'RuleType', 'repeatedAssignment');
%-------------------------------------------------------------
%addrule(m1, 'JFlux = GmaxFlux*Nchannels/Kaff', 'RuleType', 'repeatedAssignment');
%-------------------------------------------------------------
%PAIN
...
%-------------------------------------------------------------%-------------------------------------------------------------
 m1.addparameter('PainScore', 'Value', 0, 'Units', 'dimensionless', ...
    'Constant', 0);
 m1.addparameter('PainThreshold', 'Value', 100, 'Units', 'nanomolarity/hour', ...
    'Constant', 0);
  m1.addparameter('scalePain', 'Value', 1, 'Units', 'nanomolarity', ...
    'Constant', 0);

 m1.addparameter('HPain', 'Value', 1, 'Units', 'dimensionless', ...
    'Constant', 0);
addrule(m1, 'PainScore = Kaff * NchSig/scalePain^2', 'RuleType', 'repeatedAssignment');
